...
from crdesigner.map_conversion.sumo_map.util import _erode_lanelets, _find_intersecting_edges
...
class ScenarioFixer:
    ...
    def _find_intersecting_lanelets(self,
                                    lanelet_network: LaneletNetwork, in_lanelets: Dict[tuple, List[int]],
        ...
        incoming_dict = {(inter.intersection_id, inc.incoming_id): inc.incoming_lanelets
                         for inter in lanelet_network.intersections for inc in inter.incomings}
        ...
        for incoming_id, lanelets_in in in_lanelets.items():
            for out_id, lanelets_out in out_lanelets.items():
                if incoming_id[0] != out_id[0]: continue  # check only inc/out of same intersection
            ...
            for i, inc in enumerate(inter.incomings[:-1]):
                for inc2 in inter.incomings[i + 1:]:
                    for l1 in inc.incoming_lanelets:
                        if l1 in inc2.incoming_lanelets or \
                                (self.lanelet_network.find_lanelet_by_id(l1).adj_left is not None and
                                 self.lanelet_network.find_lanelet_by_id(l1).adj_left in inc2.incoming_lanelets) or \
                                (self.lanelet_network.find_lanelet_by_id(l1).adj_right is not None and
                                 self.lanelet_network.find_lanelet_by_id(l1).adj_right in inc2.incoming_lanelets):
    ...
    def _find_intersection_points(self, lanelet_network: LaneletNetwork,
                                  intersecting_incomings: Dict[Tuple[int, int], List[Tuple[int, int]]]):
        lines = {}
        ...
        for inc, lanelets_intersecting in intersecting_incomings.items():
            ...
            for l_in, l_out in lanelets_intersecting:
                if l_in not in lines:
                    lines[l_in] = [LineString(lanelet_network.find_lanelet_by_id(l_in).left_vertices),
                ...
                if l_out not in lines:
                    lines[l_out] = [LineString(lanelet_network.find_lanelet_by_id(l_out).left_vertices),
    ...
    def _cut_lanelet(self, lanelet: Lanelet, point: Point, margin: float = 0.0, next_direction="left", use_point=True):
        ...
        if cut_index <= 2:
            vertices = [lanelet.center_vertices[cut_index, :],
    ...
    def _cut_lanelet_section_at_point(self, scenario: Scenario, lanelet_id: int, point: Point, margin: float) -> Dict[
        int, int]:
        """
        ...
        inner_lanelet = min(lanelets, key=lambda l: ln.find_lanelet_by_id(l).inner_distance[-1])
        adj = ["left", "right"]
        ...
        for l, l_adj in zip(new_lanelets[:-1], new_lanelets[1:]):
            scenario.lanelet_network.add_lanelet(l)
            setattr(l, f"adj_{adj[0]}", l_adj.lanelet_id)
            setattr(l_adj, f"adj_{adj[1]}", l.lanelet_id)
            setattr(l, f"adjacent_{adj[0]}_same_direction", True)
    ...
    def _replace_intersection_lanelet_references(self, replace_dict: Dict[int, int],
                                                 replace_incomings: bool = True, replace_successors: bool = True):
        """
        ...
        for inter in self.lanelet_network.intersections:
            for inc in inter.incomings:
                ...
                if replace_successors is True:
                    for id_set in [inc._successors_left, inc._successors_straight,
                                   inc._successors_right]:
                ...
                if replace_incomings is True:
                    for old_id in replace_dict.keys() & inc.incoming_lanelets:
                        replace_tmp = {succ: old_id for succ in
                                       self.lanelet_network.find_lanelet_by_id(old_id).successor}
    ...
    def _cut_incoming(self, point: Point, intersection: Intersection,
                      incoming_element: IntersectionIncomingElement,
                      scenario: Scenario,
                      margin: float):
        replacement_mapping = self._cut_lanelet_section_at_point(scenario,
    ...
    def autofit_long_incomings(self, margin=2.0):
        ...
        lanelet_dict = dict(zip(all_lanelets, [[l] for l in all_lanelets]))
        ...
        for l_id in all_lanelets:
            ...
            for l_id_other in getattr(lanelet_tmp, dir):
                ...
                if merge_dir == "predecessor":
                    merge_list.reverse()
                self.scenario._lanelet_network._lanelets[l_id] = \
                    Lanelet.merge_lanelets(merge_list[0], merge_list[1])
            ...
            for i, inc in enumerate(inter.incomings):
                ...
                for j, inc2 in enumerate(inter.incomings):
                    if j == i or inc2 in delete_incomings:
                        continue
                    if inc_set <= inc2.incoming_lanelets and \
                        inc.successors_left <= inc2.successors_left and \
                        inc.successors_straight <= inc2.successors_straight and \
                        inc.successors_right <= inc2.successors_right:

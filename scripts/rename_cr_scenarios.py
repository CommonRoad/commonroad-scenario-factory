...
for cr_file in glob.glob(os.path.join(foldername, "**/*.cr.xml"), recursive=True):
    ...
    if scene_id_orig not in map_id_dict:
        map_id_dict[scene_id_orig] = 1 + max((map_id_new for [country_id, map_name, _], map_id_new in map_id_dict.items()
    ...
    files = list(glob.glob(os.path.join(scenario_folder_tmp, "*.xml"))) + list(glob.glob(os.path.join(scenario_folder_tmp, "*.cfg")))
    for f in files:
    ...
    shutil.copy(os.path.join(solution_folder, f"solution_KS1:TR1:{scenario_id_str_old}:2020a.xml"),
                solution_path_new)

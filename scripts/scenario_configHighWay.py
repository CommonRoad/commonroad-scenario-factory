...
class ScenarioConfigHighway(ScenarioConfig):
    ...
    min_ego_velocity = 25 / 3.6  # [m/s] velocity must exceed this value at least once
    min_vehicles_in_range = 1  # min. number of vehicles in range_min_vehicles
    range_min_vehicles = 50  # [m]

    # TURNING DETECTION
    turning_detection_threshold: float = np.deg2rad(50)  # [deg] when orientation differs above threshold, a turn is detected
    ...
    braking_detection_threshold_hold: int = 3  # number of time steps for which threshold has to be hold at least

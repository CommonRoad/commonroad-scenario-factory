...
class TestGlobetrotterPipeline:
    def test_globetrotter_pipeline_extracts_exactly_one_intersection(self):
        input_maps_folder = Path(scenario_factory.__file__).parent.parent.joinpath(
            "files/input_maps"
        )
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            execution_result = globetrotter_pipeline.execute(
                [region], ctx, num_threads=1, num_processes=1
            )
...
class TestScenarioGeneration:
    ...
    def test_scenario_generation_and_globetrotter_with_pipeline_creates_one_scenario(self):
        scenario_factory_config = ScenarioFactoryConfig(
            seed=100, simulation_steps=600, cr_scenario_time_steps=75
        )
        input_maps_folder = Path(scenario_factory.__file__).parent.parent.joinpath(
            "files/input_maps"
        )

...
from scenario_factory.globetrotter import RegionMetadata, Coordinates, BoundingBox
...
REGIONAL_MAPS_TEST_DATASET = Dataset(
    initial_entries=[
        RegionalMapsTestCase(label="no_maps", map_files=[],
                             region=RegionMetadata("DE", "Aachen", 0, Coordinates(0, 0)), expected_region_map=None),
        RegionalMapsTestCase(label="single_correct_map", map_files=["aachen-latest.osm.pbf"],
                             region=RegionMetadata("DE", "Aachen", 0, Coordinates(0, 0)),
                             expected_region_map="aachen-latest.osm.pbf"),
        RegionalMapsTestCase(label="multiple_maps_with_correct",
        ...
        RegionalMapsTestCase(label="multiple_maps_without_correct",
                             map_files=["düren-latest.osm.pbf", "bonn-latest.osm.pbf", "aachen-latest.osm.pbf"],
...
MAP_EXCERPTS_TEST_DATASET = Dataset(
    initial_entries=[
        MapExcerptTestCase(label="partial_excerpt",
                           bounding_box=BoundingBox(west=6.464703, south=51.025325, east=6.486954, north=51.035756),
                           input_map="düren-latest.osm.pbf", expected_excerpt="düren-partial-excerpt.osm"),
        MapExcerptTestCase(label="complete_excerpt",
                           bounding_box=BoundingBox(west=6.445126, south=51.015997, east=6.451094, north=51.018542),
                           input_map="düren-latest.osm.pbf", expected_excerpt="düren-complete-excerpt.osm"),
        MapExcerptTestCase(label="empty_excerpt",
                           bounding_box=BoundingBox(west=6.431527, south=51.047268, east=6.442585, north=51.053013),
                           input_map="düren-latest.osm.pbf", expected_excerpt=None),
    ],
...
CENTER_POLYLINES_TEST_DATASET = Dataset(
    initial_entries=[
        CenterPolylinesTestCase(label="single_lanelet", lanelet_network=UsefulLaneletNetworks.single_lanelet_no_meta(),
                                expected_center_polylines={1: np.array([[0, -5], [25, -5], [25, 20]])}),
        CenterPolylinesTestCase(label="empty", lanelet_network=UsefulLaneletNetworks.empty_no_meta(),
                                expected_center_polylines={}),
        CenterPolylinesTestCase(label="one_split", lanelet_network=UsefulLaneletNetworks.one_split_no_meta(),
                                expected_center_polylines={
                                    1: np.array([[0, -5], [20, -5]]),
...
LOCAL_PROVIDER_TEST_DATASET = Dataset(
    initial_entries=[
        LocalProviderTestCase(label="generic1",

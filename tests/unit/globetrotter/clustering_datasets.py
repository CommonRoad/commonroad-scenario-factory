...
class CentroidTestCase(TestCase):
    labels: np.ndarray
    points: np.ndarray
    expected_result: dict[int, tuple[np.ndarray, float, np.ndarray]]   # Map: cluster -> (centroid, distance, points)
...
CLUSTERING_TEST_DATASET = Dataset(
    initial_entries=[
        ClusteringTestCase(label="generic1", points=np.array([[0, 0], [2, 0], [0, 3], [60, 0], [63, 0], [61, 3]]),
                           expected_labels=np.array([1, 1, 1, 0, 0, 0])),
        ClusteringTestCase(label="generic2", points=np.array([[0, 0], [2, 0], [0, 2], [60, 0], [62, 0], [60, 2],
                                                              [60, 60], [62, 60], [60, 62]]),
...
OUTER_DISTANCE_TEST_DATASET = Dataset(
    initial_entries=[
        OuterDistanceTestCase(label="generic1", cluster=np.array([[0, 0], [1, 1], [3, 1]]),
                           center=np.array([1, 1]), expected_distance=2),
        OuterDistanceTestCase(label="generic2", cluster=np.array([]),
                           center=np.array([0, 0]), expected_distance=0)
    ],
...
CENTROID_TEST_DATASET = Dataset(
    initial_entries=[
        CentroidTestCase(label="generic1", labels=np.array([2, 2, 2, 1, 1, 1, 0, 0, 0]), points=np.array(
...
FORKING_POINTS_TEST_DATASET = Dataset(
    initial_entries=[
        ForkingPointsTestCase(label="empty_network", lanelet_network=UsefulLaneletNetworks.empty_no_meta(),
                              expected_forking_points=np.array([])),
        ForkingPointsTestCase(label="one_split_network", lanelet_network=UsefulLaneletNetworks.one_split_no_meta(),
                              expected_forking_points=np.array([[20, -5]])),
        ForkingPointsTestCase(label="malformed_one_split_network",

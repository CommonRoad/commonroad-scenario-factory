...
class GenerateCRScenarios:
    ...
    def _default_ego_selection_criteria(self) -> List[Callable]:
        ...
        mapping = {
            EgoSelectionCriterion.turning: self.turning_criterion,
    ...
    def _choose_ego_from_obstacles(self, planning_pro_per_scen: int, obstacles: Dict[int, DynamicObstacle]):
        ...
        for ego_id, init_time in ego_selected.items():  # obs: List[Obstacles]
            ...
            ego = obstacles_copy[ego_id]
            assert (
                ego_id in obstacles_copy
            ), "UNEXPECTED ERROR: default_ego_id {} not in obstacles anymore, deleted?".format(ego_id)
        ...
        for obs_id, obs in obstacles.items():
            if init_time >= obs.initial_state.time_step:
                ...
                state_list = copy.deepcopy(
                    [
                        obs.prediction.trajectory.state_at_time_step(t)
                        for t in range(init_time + 1, end_time)
                ...
                new_obs[obs_id] = DynamicObstacle(
                    copy.deepcopy(obs.obstacle_id),
                    obs.obstacle_type,
                    obstacle_shape=obs.obstacle_shape,
                    initial_state=initial_state,
                    prediction=TrajectoryPrediction(
                        Trajectory(initial_time_step=1, state_list=state_list), shape=obs.obstacle_shape
                    ),
                )
            pass
        return new_obs
    ...
    def create_planning_problem(
        ...
        for i in range(num_planning_pro):
            ...
            goal_state = InitialState(
                time_step=Interval(last_state.time_step - 1, last_state.time_step),
                    ...
                    if len(lanelet_of_goal_position) > 1:
                        ...
                        for l_goal in lanelet_of_goal_position:
                            for l_init in initial_lanelets:
                                dist = self.scenario_model.lanelet_section_network.get_shortest_distance_lanelets(
                                    l_init, l_goal
                                )
                                if dist < dist_min:
    ...
    def _select_as_ego(self, vehicle: DynamicObstacle, selection_function="v_diff"):
        ...
        :param selection_function: 'v_diff': velocity difference of trajectory; 'accel': acceleration of vehicle
    ...
    def _remove_irrelevant_obs_ids(self, ego: DynamicObstacle, init_time, end_time, obstacles_copy):
        ...
        for time_step, veh_dict in self.state_dict.items():
            ...
            proj_pos[0] += cos(veh_dict[ego.obstacle_id].orientation) + 2.0 * veh_dict[ego.obstacle_id].velocity
            proj_pos[1] += sin(veh_dict[ego.obstacle_id].orientation) + 2.0 * veh_dict[ego.obstacle_id].velocity
            if init_time <= time_step <= end_time:
                for veh_id, state in veh_dict.items():
                    if veh_id not in relevant:
                        if np.less_equal(
                            np.abs(state.position[0] - proj_pos[0]), self.conf_scenario.sensor_range
                        ) and np.less_equal(np.abs(state.position[1] - proj_pos[1]), self.conf_scenario.sensor_range):
    ...
    def write_final_cr_file(
        ...
        if planning_problem_set is not None:
            fw = CommonRoadFileWriter(
                ...
                author=self.conf_scenario.author,
                affiliation=self.conf_scenario.affiliation,
                source=self.conf_scenario.source,
                tags=tags,
                decimal_precision=4,
            )
            ...
            file_writer = CommonRoadFileWriter(commonroad_scenario, problemset, tags, decimal_precision=4)
    ...
    def write_interactive_scenarios_and_videos(
        self,
        ...
        for k in range(len(self.list_cr_scenarios)):
            ...
            assert deleted_vehicle is True, "ego vehicle not deleted"
            CR2SumoMapConverter.generate_cfg_file(
                str(commonroad_scenario.scenario_id),
                net_file=net_file,
                route_files=rou_files_new,
            ...
            self.write_interactive_scenario_files(
                dir_name,
            ...
            if self.conf_scenario.save_ego_solution_file:
                ...
                traj = Trajectory(
                    initial_time_step=0,
                    state_list=[self.list_ego_obstacles[k].initial_state]
                    + self.list_ego_obstacles[k].prediction.trajectory.state_list,
                )
                solution_pp = PlanningProblemSolution(
    ...
    def write_interactive_scenario_files(
        self,
        dirname,
        init_time_step: int,
        ...
        fw = CommonRoadFileWriter(
            commonroad_scenario_init_state,
            planning_problem_set,
            self.conf_scenario.author,
            self.conf_scenario.affiliation,
            self.conf_scenario.source,
            tags,
            decimal_precision=4,
        )
        fw.write_to_file(filename, OverwriteExistingFile.ALWAYS, check_validity=check_validity)
        ...
        set_attributes = {
            "presimulation_steps": simulation_config.presimulation_steps + init_time_step,
        ...
        with open(os.path.join(dirname, "simulation_config.p"), "wb") as f:
            ...
            for attr in dir(interactive_base_config):
                if attr.startswith("__") or callable(getattr(out_config, attr)):
                    continue
                if (
                    hasattr(interactive_base_config, attr)
                    and getattr(interactive_base_config, attr) == ParamType.NOT_SET
                ):
    ...
    def select_ego_vehicles(
        ...
        """
        Returns ids of vehicles that are chosen as ego vehicle and optionally a time step where the interesting maneuver
        begins.
        ...
        for obs_id, time_steps in ego_veh_init_times.items():
            ...
            filtered_list = []
            if (
                obstacles[obs_id].prediction.trajectory.final_state.time_step
            ...
            for init_time in time_steps:
                ...
                final_lanelets = list(
                    obstacles[obs_id].prediction.center_lanelet_assignment[
                ...
                rear_vehicles, front_vehicles = self.scenario_model.get_array_closest_obstacles(
                    obstacles[obs_id],
    ...
    def _acceleration_heuristic(self, obstacle: DynamicObstacle) -> Tuple[bool, Union[None, int]]:
        ...
        return self._threshold_and_max_detection(
            accelerations,
            initial_timestep=obstacle.initial_state.time_step,
    ...
    def _threshold_and_lag_detection(
        self, signal: np.ndarray, initial_timestep: int, threshold: float, lag_threshold: float
    ...
    def _threshold_and_max_detection(
        self, signal: np.ndarray, threshold: float, initial_timestep: int, time_gap: float = 1.0, n_hold: int = 2
    ):
        """
    ...
    def get_number_of_veh_in_range(
        self,
        position: np.ndarray,
        time_step: int,
        range_min_vehicles,

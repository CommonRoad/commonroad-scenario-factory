...
class CommonRoadDB:
    ...
    def make_new_id(self, original_id: Union[List[str], str], existing_map=True) -> Tuple[DataFrame, int]:
        # create new id
        if not isinstance(original_id, list):
            row_data = self._parse_scenario_id(original_id)
        else:
            row_data = original_id

        if existing_map is True:
            request: DataFrame = self.db.loc[
                (self.db[ScenarioKeys.cooperative] == row_data[0])
    ...
    def _create_file_for_new_scenario(
        self, new_row: DataFrame, scenario: Scenario, planning_problem_set: PlanningProblemSet, new_path
    ):
        # assemble meta data
        ...
        file_writer.write_to_file(
            os.path.join(new_path, scenario.benchmark_id + ".xml"), overwrite_existing_file=OverwriteExistingFile.ALWAYS
        )
    ...
    def create_new_scenario(
        ...
        folder_path: str,
        original_id: Union[List[str], str] = None,
        identical_map=False,
        new_id: str = None,
        add_authors: List[str] = None,
        add_tags=None,
        delete_tags=None,
        source=None,
        affiliation=None,

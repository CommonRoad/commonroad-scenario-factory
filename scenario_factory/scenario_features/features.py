...
def extract_features(scenario, planning_problem_set, ego_vehicle):
    ...
    if ego_vehicle is not None:
        ...
        min_ttc, min_ttc_ts = get_min_ttc(
            scenario, scenario.lanelet_network, ego_vehicle
        )  # TODO might not be correct or accurate
...
def remove_redundant_state_fields(init_state, example_state: State) -> State:  # Non KS fields
    vals = {attr: getattr(init_state, attr, 0.0) for attr in example_state.attributes}
    return State(**vals)
...
def get_args():
    ...
    parser.add_argument(
        "--ego", action="store_false", help="Set if the scenario contains ego trajectory as an obstacle"
    )

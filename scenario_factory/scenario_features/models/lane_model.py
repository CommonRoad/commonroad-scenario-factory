...
class LaneletSection:
    ...
    def __init__(self, lanelet_list: List[Lanelet], section_id: SectionID, succ_section: Set[int] = None,
                 pred_section: Set[int] = None):
        # stores lanelets starting with the rightmost one
        self.lanelet_list: List[Lanelet] = lanelet_list
        self.section_id = section_id
        self.lanelet_lengths = [l.distance[-1] for l in lanelet_list]
...
class LaneletSectionNetwork:
    ...
    def __init__(self, lanelet_sections: List[LaneletSection], debug_plots=False):
        ...
        lanelet_list = itertools.chain.from_iterable([l.lanelet_list for l in lanelet_sections])
        self.lanelets: Dict[int, Lanelet] = {l.lanelet_id: l for l in lanelet_list}
    ...
    def get_coordinate_system_lanelet(self, lanelet_id: int):
        l = self.lanelets[lanelet_id]
        csys = CurvilinearCoordinateSystem(smoothen_polyline(l.center_vertices, resampling_distance=0.5))
        point = csys.projection_domain()[0] * 0.5 + csys.projection_domain()[10] * 0.5
        try:
            self._s_interval[lanelet_id] = Interval(csys.convert_to_curvilinear_coords(l.center_vertices[0][0],
                                                                                       l.center_vertices[0][1])[0],
                                                    csys.convert_to_curvilinear_coords(l.center_vertices[-1][0],
                                                                                       l.center_vertices[-1][1])[0]
                                                    )
        except ValueError:
            print(l.center_vertices)
            # print(csys.projection_domain())
            print(smoothen_polyline(l.center_vertices, resampling_distance=0.5))
            self.debug_plot_curv_projection(l.center_vertices[0], csys,
                                            smoothen_polyline(l.center_vertices, resampling_distance=0.5))
        # #TODO delete
    ...
    def get_curv_position_section(self, position: np.ndarray, section_id: SectionID) -> np.ndarray:
        ...
        try:
            pos_c = self.get_coordinate_system_section(section_id).convert_to_curvilinear_coords(position[0],
                                                                                                 position[1])
        except ValueError:
    ...
    def compute_longitudinal_distance(self, long_0: float, long_1: float, section_0: SectionID, section_1: SectionID,
    ...
    def get_distance_on_route(self, long_0: float, long_1: float, section_route: List[SectionID],
                              distance_type: str = 'min'):
        """
    ...
    def create_lanelet2section_id(lanelet_sections: List[LaneletSection]) -> Dict[int, SectionID]:
        ...
        for ls in lanelet_sections:
            for l in ls.lanelet_list:
                lanelet2section_id[l.lanelet_id] = ls.section_id
    ...
    def add_section(self, lanelet_section: LaneletSection):
        ...
        self.lanelets.update({l.lanelet_id: l for l in lanelet_section.lanelet_list})
    ...
    def add_to_graph(self, lanelet_section: LaneletSection):
        ...
        for p in lanelet_section.pred_sections:
            self._graph.add_edge(p, section_id,
        ...
        for s in lanelet_section.succ_sections:
            self._graph.add_edge(section_id, s,
    ...
    def _update_successors(self, lanelet_section: LaneletSection):
        ...
        for l in lanelet_section.lanelet_list:
            if l.successor is not None:
                successor_lanelets = successor_lanelets.union(list(l.successor))
    ...
    def _update_predecessors(self, lanelet_section: LaneletSection):
        ...
        for l in lanelet_section.lanelet_list:
            if l.predecessor is not None:
                predecessor_lanelets = predecessor_lanelets.union(list(l.predecessor))
    ...
    def debug_plot_curv_projection(self, position, cosy: CurvilinearCoordinateSystem, reference_path=None):
        ...
        LaneletNetwork.create_from_lanelet_list(list(self.lanelets.values())).draw(rnd, draw_params={'lanelet': {'show_label': True}})
...
class SectionRoute:
    ...
    def append(self, new_section: LaneletSection) -> None:
        if len(self.lanelet_sections) > 0:
            ...
            new_lanelets = [l.lanelet_id for l in new_section.lanelet_list]
            for l in self.lanelet_sections[-1].lanelet_list:
                if l.successor is not None and len(set(l.successor).intersection(new_lanelets)) > 0:
                    connected_lanelets = set(l.successor).intersection(new_lanelets)
                    for connecting_index, connecting_lanelet in enumerate(new_section.lanelet_list):
                        if connecting_lanelet.lanelet_id in connected_lanelets:
                            connecting_index -= self.lateral_indices[l.lanelet_id]
                            break
